...
COLLECTION_LOG = os.path.join(SCRIPT_DIR, f'{TODAY_STR}_collection_log.csv')
UPDATE_HISTORY = os.path.join(SCRIPT_DIR, 'update_history.csv')
...
def collect_incremental(tickers, ticker_name_map, existing_cache, last_dates):
    ...
    for tk in tickers:
        if tk not in existing_cache:
            ...
            last = last_dates[tk]
            # 마지막 데이터가 오늘이거나 어제(주말/공휴일 고려)면 스킵
            if (TODAY_DATE - last.date()).days <= 1:
