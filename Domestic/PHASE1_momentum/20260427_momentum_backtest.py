...
# 고정 캐시 파일명 우선, 없으면 날짜별 파일 탐색
DAILY_CACHE = os.path.join(SCRIPT_DIR, 'daily_cache.pkl')
WEEKLY_CACHE = os.path.join(SCRIPT_DIR, 'weekly_cache.pkl')
...
# 출력
OUTPUT_BACKTEST = os.path.join(SCRIPT_DIR, f'{TODAY_STR}_momentum_backtest.xlsx')
OUTPUT_DD_DIST = os.path.join(SCRIPT_DIR, f'{TODAY_STR}_dd_forward_distribution.xlsx')
CHART_DIR = os.path.join(SCRIPT_DIR, f'{TODAY_STR}_charts')
...
def run_all_backtests(data_dict, sector_map, frame_label, forward_window):
    ...
    for period_name, n_days in PERIOD_MAP.items():
        ...
        print(f"  DD: {len(dd_rows)}, MACD: {len(macd_rows)}, RSI: {len(rsi_rows)}, 52W: {len(h52_rows)}")
...
def generate_summary_charts(all_results):
    ...
    for key, df in all_results.items():
        ...
        elif 'cum_return_pct' in df.columns:
