"""
PHASE1 Momentum - 일일 실행 러너
Description:
    매일 저녁 실행하면 Step 1→2→3를 순차 실행합니다.
    - Step 1: 데이터 증분 업데이트 (신규 데이터만 추가, 1~5분)
    - Step 2: 백테스트 재실행 (5~15분)
    - Step 3: 랭킹 재산출 (1분 이내)

Usage:
    C:/Python311/python.exe run_daily.py
    C:/Python311/python.exe run_daily.py --skip-collect   # 데이터 수집 스킵
    C:/Python311/python.exe run_daily.py --only-collect    # 데이터 수집만
"""
...
STEPS = [
    ('Step 1: 데이터 수집 (증분)', '20260427_data_collector.py'),
